    ...
    while (T_now <= T_sim) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t i = 0; i < Neu_adapt.size(); ++i) {
                Neu_res[Neu_res.size()-i].adapt(Neu_adapt[i]);
            ...
            #pragma omp section
            {
                ...
                for (size_t i = 0; i < Neu_res.size(); ++i) {
                    if (Neu_res[i].is_firing()) {
                        ...
                        if (i > Neu_res.size() - Neu_adapt.size()) Neu_adapt[i].up();
                        Neu_res[i].reset();
                    }
                }
            }
...
// Load bias spike train
void Core::load_bias_spike_train(const std::vector<uint32_t>& spike_times, const std::vector<uint16_t>& neuron_indices) {
    ...
    // 각 뉴런별로 spike train을 뒤집을지 결정하는 랜덤 플래그
    ...
    const uint32_t MAX_TIME = 1000000; // 1초 = 1,000,000 마이크로초
    
    for (size_t i = 0; i < spike_times.size(); ++i) {
        uint32_t final_time = spike_times[i];
        if (should_flip[random_indices[neuron_indices[i]]]) {
            final_time = MAX_TIME - spike_times[i];
        }
        external_S_queue.push(Spike(final_time, {random_indices[neuron_indices[i]], 'b'}));
    }
}

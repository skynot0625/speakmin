...
class GSCD(object):
    ...
    def random_split(self, category_list, split_number_list, rng=None, rng_seed=10, wav_file_source='all', use_all_in_source=False):
        try:
            ...
            print(f"Error in random_split: {str(e)}")
            return []
        finally:
            gc.collect()
    ...
    def _worker_queue(cls, queue, result_queue):
        while True:
            try:
                ...
                spikes = sound.speech2spikes_realtime(
                    item['n_mels'],
                    item['vth'],
                    alpha=item['alpha'],
                    leak_enable=item.get('leak_enable', False),
                    leak_tau=item.get('leak_tau', 16000e-6)
                )
